...
class TryOver3::A1
  def method_missing(method, *args)
    return true if method.match(/^test_/)
    super
  end
...
class TryOver3::A2Proxy
  ...
  def initialize(try_over_a2_instance)
    self.source = try_over_a2_instance
  end
  ...
  def respond_to_missing?(sym, include_private)
    source.respond_to?(sym) ? true : super
  end
end
...
class TryOver3::A4
  ...
  def self.const_missing(const_name)
    return super if (@runners & [const_name]).empty?
  ...
  def self.included(klass)
    klass.define_singleton_method :task do |name, &task_block|
      ...
      define_singleton_method :const_missing do |const_name|
        if respond_to?(const_name.to_s.downcase)
          Class.new do
            define_singleton_method :run do
              warn "Warning: #{klass}::#{const_name}.run is deprecated"
              runner.call
            end
          end
        else
      ...
      define_singleton_method name, runner
    end
  end
end
