...
class EvilMailbox
  ...
  def initialize(mail_obj, secret_char = nil)
    self.mail_obj = mail_obj

    if secret_char
      mail_obj.auth(secret_char)
    end

    define_singleton_method :send_mail do |to, body, &block|
      if block
        block.call(mail_obj.send_mail(to, "#{body}#{secret_char}"))
      else
        mail_obj.send_mail(to, "#{body}#{secret_char}")
      end
